...
dcid: "wikidataId/{wiki_id}"
"""


def generate(in_district, out_path):
    ...
    with open(in_district) as fin:
        with open(os.path.join(out_path, "JK_Districts_GeoJson.mcf"),
                  'w') as fout:
    ...
    for f in j['features']:
        ...
        fout.write(
            _MCF_FORMAT.format(ext_id=census2011,
                               ...
                               wiki_id = f['properties']['wikidataId'],
                               gj_str=gj))


def main(_):
    generate(FLAGS.input_district_geojson,
